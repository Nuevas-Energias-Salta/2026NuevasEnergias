...
def publicar_en_github(informes_generados):
    ...
    # Obtener el mes actual para el histórico (formato YYYY-MM)
    ...
    for info in informes_generados:
        ...
        # 1. Copiar como index.html (último informe)
        shutil.copy2(info['filepath'], carpeta_cliente / "index.html")
        
        # 2. Copiar como histórico (con fecha del periodo reportado)
    ...
    # Crear un index.html básico en la raíz para que no de 404 la home
...
def calcular_metricas_nis(datos, nis):
    ...
    # Ãšltimo registro (el más reciente está en la fila 2, primera en la lista)
    ...
    for reg in registros_nis:
        try:
            ...
            # Consumo total = Consumo de la red + Generación FV
    ...
    # Función para formatear fecha a nombre de mes (abreviado)
    def formatear_mes(fecha_str):
        ...
        # Mapeo de abreviaturas a números
        mes_map = {
            'ene': '01', 'jan': '01',
            'feb': '02',
            'mar': '03',
    ...
    # Datos históricos para gráficos TRIMESTRAL (últimos 3 meses)
    historico_labels_3m = []
    ...
    for reg in registros_nis[:3]:
        try:
            fecha = reg.get("   Fecha ", "").strip()  # 3 espacios antes, 1 después
    ...
    # Datos históricos para gráficos ANUAL (últimos 12 meses)
...
def generar_html_informe(metricas):
    """Genera el HTML del informe con gráficos interactivos Chart.js optimizados."""
    ...
    # --- LOGICA BUSINESS (Growth) ---
    # Calcular Benchmarking
    if len(metricas['historico_consumo_3m']) >= 2:
        # Indice 0 es el mas antiguo, el ultimo es el mas reciente (se invirtio en calcular_metricas_nis)
        # Check carefully: it was reversed so [-1] is current month, [-2] is previous
        consumo_actual = metricas['historico_consumo_3m'][-1] + metricas['historico_generacion_3m'][-1]
        consumo_anterior = metricas['historico_consumo_3m'][-2] + metricas['historico_generacion_3m'][-2]
        if consumo_anterior > 0:
            variacion_consumo = ((consumo_actual - consumo_anterior) / consumo_anterior) * 100
            if variacion_consumo < 0:
                texto_bench = f"🏆 Tu consumo total bajó un <strong>{abs(variacion_consumo):.1f}%</strong> respecto al mes pasado. ¡Excelente!"
                clase_bench = "bench-good"
            else:
                texto_bench = f"📈 Tu consumo total subió un <strong>{variacion_consumo:.1f}%</strong> respecto al mes pasado."
                clase_bench = "bench-warn"
        else:
            texto_bench = "Midiendo tendencia de consumo..."
            clase_bench = "bench-neutral"
    else:
        texto_bench = "Midiendo tendencia de consumo..."
        clase_bench = "bench-neutral"

    # Banner Dinámico de Temporada (Cross-Selling)
    from datetime import datetime
    mes_actual_num = datetime.now().month
    if 3 <= mes_actual_num <= 8:
        # Otoño/Invierno (Target T2: Termostatos Smart / Calefacción Solar)
        banner_titulo = "❄️ ¿Preparado para el frío?"
        banner_texto = "¡Ahorra en tu calefacción! Convierte tu Piso Radiante en inteligente con nuestros Termostatos WiFi sin obras. Responde a nuestros emails para pedir cotización."
        banner_tema = "invierno"
    else:
        # Primavera/Verano (Target T2: Bombas Piletas / Aire Libre)
        banner_titulo = "☀️ Maximiza tu verano"
        banner_texto = "¿Tienes piscina? Alarga la temporada con nuestras Bombas de Calor, ¡gratis aprovechando el sol! Aprovecha tu instalación FV al máximo."
        banner_tema = "verano"
    # --------------------------------
...
<head>
    ...
    <style>
        * {{
        ...
        /* NUEVOS ESTILOS: BENCHMARK Y BANNER CROSS-SELLING */
        .bench-card {{
            background: white;
            padding: 16px;
            border-radius: 8px;
        ...
        .bench-good {{ border-left: 5px solid #4CAF50; }}
        .bench-warn {{ border-left: 5px solid #FF9800; }}
        .bench-neutral {{ border-left: 5px solid #2196F3; }}
...
<body>
    <div class="container">
        <div class="header">
            <div class="period-toggle">
                ...
                <button class="period-btn" onclick="switchPeriod('12m')">Año</button>
            </div>
            <h1 id="headerTitle">INFORME FV ÃšLTIMOS 3 MESES</h1>
            <div class="nis">NIS: {metricas['nis']} | Cliente: {metricas['cliente']} | Categoría: {metricas['categoria']}</div>
        </div>
        
        <!-- SECCION BENCHMARK -->
        <div class="bench-card {clase_bench}">
            {texto_bench}
        </div>
        ...
        <div class="promo-banner {banner_tema}">
            <div class="promo-title">{banner_titulo}</div>
            <div class="promo-text">{banner_texto}</div>
        </div>
    ...
    <script>
        ...
        // Gráfico 1: Barras Apiladas + Línea
        ...
        const chartBarras = new Chart(ctxBarras, {{
            ...
            data: {{
                ...
                datasets: [
                    ...
                    {{
                        label: 'Generación FV (Autoconsumida)',
                    ...
                    tooltip: {{
                        ...
                        callbacks: {{
                            label: function(context) {{
                                ...
                                // Calcular porcentaje solo para barras (no para la línea de total)
                            ...
                            afterLabel: function(context) {{
                                // Mostrar resumen en el último dataset
...
"""
    ...
    return html
    """Genera el HTML del informe con gráficos matplotlib."""
    ...
    # Línea de generación
    ax1.plot(x, generacion, label='Generación FV [kWh]', color='#000000', 
             linewidth=2, marker='o', markersize=6, zorder=10)
    
    # Configuración
    ax1.set_xticks(x)
    ax1.set_xticklabels(labels, fontsize=10)
    ax1.set_ylabel('Generación y Consumo en kWh', fontsize=10)
    ...
    # ============================================
    # GRÁFICO 2: Gauge COâ‚‚ (Semicírculo)
    # ============================================
    ...
    theta = [0, (co2_percent / 100) * 180, 180]  # 0 a 180 grados (semicírculo)
    radii = [1, 1, 1]
    width = [0.4, 0.4, 0.4]
...
<body>
    <div class="container">
        ...
        <div class="main-layout">
            <div class="chart-container">
                <img src="{chart1_base64}" alt="Gráfico de Consumo y Generación">
            </div>
...
"""
    ...
    return html
    """Genera el HTML del informe con gráficos interactivos Chart.js."""
    ...
    co2_value = int(metricas['co2_evitado'])
    co2_max = max(5000, co2_value * 1.2)  # Escala dinámica
            ...
            options: {{
                ...
                scales: {{
                    ...
                    y: {{
                        ...
                        title: {{
                            display: true,
                            text: 'Generación y Consumo en kWh'
                        }},
                        ticks: {{
                            callback: function(value) {{
...
"""
    ...
    # QuickChart requiere configuración específica para charts mixtos
    chart1_config = {
        "type": "bar",
        "data": {
            "labels": metricas['historico_labels'],
    ...
    # GRÁFICO 2: Gauge semicircular para COâ‚‚
